...
public class TypelessFeedServerClient {
   ...
   * malformed.
   */
  public Map<String, Object> insertEntry(URL baseUrl, Map<String, Object> mapEntry) throws
      FeedServerClientException {

    try {
      log.info("inserting entry to feed " + baseUrl);
      FeedServerEntry entry = service.insert(baseUrl, getEntryFromMap(mapEntry));
      return getMapFromEntry(entry);
    } catch (MalformedURLException e) {
      throw new RuntimeException("Invalid URL", e);
    } catch (IOException e) {
      throw new FeedServerClientException("Error while inserting " + baseUrl, e);
    } catch (ServiceException e) {
      throw new FeedServerClientException(e);
    } catch (NullPointerException e) {
      throw new RuntimeException("Invalid Entry", e); 
   ...
   */
  public Map<String, Object> getMapFromXml(String xmlText) throws
      FeedServerClientException {
    try {
