...
goog.string.quote = function(s) {
  ...
  if(s.quote) {
    ...
    for(var i = 0;i < s.length;i++) {
      sb.push(goog.string.escapeChar(s.charAt(i)))
    }sb.push('"');
    return sb.join("")
  }
};
...
goog.array.indexOf = function(arr, obj, opt_fromIndex) {
  ...
  }var fromIndex = opt_fromIndex == null ? 0 : Math.max(0, arr.length + opt_fromIndex);
...
goog.array.compare = function(arr1, arr2, opt_compareFn) {
  if(!goog.isArrayLike(arr1) || !goog.isArrayLike(arr2) || arr1.length != arr2.length) {
    return false
  }var l = arr1.length, compareFn = opt_compareFn || goog.array.defaultCompareEquality;
  for(var i = 0;i < l;i++) {
...
goog.structs.Set.prototype.equals = function(col) {
  return this.getCount() != goog.structs.getCount(col) ? false : this.isSubsetOf(col)
};
...
goog.structs.CircularBuffer.prototype.getNewestValues = function(maxCount) {
  var l = this.getCount(), start = this.getCount() - maxCount, rv = [];
...
goog.debug.DebugWindow.prototype.writeInitialDocument_ = function() {
  ...
  var html = '<style>*{font:normal 14px monospace;}.dbg-sev{color:#F00}.dbg-w{color:#E92}.dbg-sh{font-weight:bold;color:#000}.dbg-i{color:#666}.dbg-f{color:#999}.dbg-ev{color:#0A0}.dbg-m{color:#990}</style><hr><div class="dbg-ev" style="text-align:center">' + this.welcomeMessage + "<br><small>Logger: " + this.identifier_ + "</small></div><hr>";
  this.writeToLog_(html);
...
goog.net.EventType = {COMPLETE:"complete", SUCCESS:"success", ERROR:"error", ABORT:"abort", READY:"ready", READY_STATE_CHANGE:"readystatechange", TIMEOUT:"timeout", INCREMENTAL_DATA:"incrementaldata"};goog.json = {};
...
};goog.events.KeyCodes = {MAC_ENTER:3, BACKSPACE:8, TAB:9, ENTER:13, SHIFT:16, CTRL:17, ALT:18, PAUSE:19, CAPS_LOCK:20, ESC:27, SPACE:32, PAGE_UP:33, PAGE_DOWN:34, END:35, HOME:36, LEFT:37, UP:38, RIGHT:39, DOWN:40, INSERT:45, DELETE:46, ZERO:48, ONE:49, TWO:50, THREE:51, FOUR:52, FIVE:53, SIX:54, SEVEN:55, EIGHT:56, NINE:57, A:65, B:66, C:67, D:68, E:69, F:70, G:71, H:72, I:73, J:74, K:75, L:76, M:77, N:78, O:79, P:80, Q:81, R:82, S:83, T:84, U:85, V:86, W:87, X:88, Y:89, Z:90, CONTEXT_MENU:93, NUM_PLUS:107, 
NUM_MINUS:109, F1:112, F2:113, F3:114, F4:115, F5:116, F6:117, F7:118, F8:119, F9:120, F10:121, F11:122, F12:123, NUMLOCK:144, COMMA:188, DOT:190, SLASH:191, BACKSLASH:220, WIN_KEY:224, WIN_IME:229, PRINT_SCREEN:44};
...
goog.events.ActionHandler.inherits(goog.events.EventTarget);
...
goog.events.ActionHandler.prototype.handleKeyDown_ = function(e) {
  if(e.keyCode == goog.events.KeyCodes.ENTER || goog.userAgent.SAFARI && e.keyCode == goog.events.KeyCodes.MAC_ENTER) {
...
goog.events.ActionHandler.prototype.dispatchEvents_ = function(e) {
  var beforeActionEvent = new goog.events.BeforeActionEvent(e), dispatched = false;
  try {
...
goog.events.ActionHandler.prototype.getParentEventTarget = function() {
  return this.parent_
};
goog.events.ActionHandler.prototype.dispose = function() {
  if(!this.getDisposed()) {
    goog.events.ActionHandler.superClass_.dispose.call(this);
    goog.events.unlisten(this.element_, goog.events.EventType.KEYDOWN, this.handleKeyDown_, false, this);
...
goog.events.ActionEvent.inherits(goog.events.BrowserEvent);
goog.events.BeforeActionEvent = function(browserEvent) {
  goog.events.BrowserEvent.call(this, browserEvent);
  this.type = goog.events.ActionHandler.EventType.BEFOREACTION
};
goog.events.BeforeActionEvent.inherits(goog.events.BrowserEvent);goog.events.KeyNames = {8:"backspace", 9:"tab", 13:"enter", 16:"shift", 17:"ctrl", 18:"alt", 19:"pause", 20:"caps-lock", 27:"esc", 32:"space", 33:"pg-up", 34:"pg-down", 35:"end", 36:"home", 37:"left", 38:"up", 39:"right", 40:"down", 45:"insert", 46:"delete", 48:"0", 49:"1", 50:"2", 51:"3", 52:"4", 53:"5", 54:"6", 55:"7", 56:"8", 57:"9", 61:"equals", 65:"a", 66:"b", 67:"c", 68:"d", 69:"e", 70:"f", 71:"g", 72:"h", 73:"i", 74:"j", 75:"k", 76:"l", 77:"m", 78:"n", 79:"o", 80:"p", 81:"q", 82:"r", 83:"s", 
84:"t", 85:"u", 86:"v", 87:"w", 88:"x", 89:"y", 90:"z", 93:"context", 107:"num-plus", 109:"num-minus", 112:"f1", 113:"f2", 114:"f3", 115:"f4", 116:"f5", 117:"f6", 118:"f7", 119:"f8", 120:"f9", 121:"f10", 122:"f11", 123:"f12", 188:",", 190:".", 191:"/", 220:"\\", 224:"win"};goog.events.KeyHandler = function(opt_element) {
  goog.events.EventTarget.call(this);
  if(opt_element) {
...
goog.events.KeyHandler.prototype.handleEvent = function(e) {
  var be = e.getBrowserEvent(), keyCode = be.keyCode, which = be.which, key = keyCode || which, keyIdentifier = be.keyIdentifier;
  if(which) {
  ...
  this.lastKey_ = key;
  if(goog.userAgent.SAFARI && repeat) {
    if(be.timeStamp - this.lastTimeStamp_ < 10) {
      return
    }this.lastTimeStamp_ = be.timeStamp
  }var event = new goog.events.KeyEvent(key, repeat, be);
  try {
...
goog.events.KeyHandler.prototype.attach = function(element) {
  if(this.keyupKey_ && this.listenKey_) {
    this.detach()
  }this.element_ = element;
...
goog.events.KeyEvent = function(keyCode, repeat, browserEvent) {
  goog.events.BrowserEvent.call(this, browserEvent);
...
google.accounts = {};
google.accounts.EXTERNAL_PREFIX_ = "https://www.google.com/accounts/";
...
google.accounts.TokenStore_.prototype.set_ = function(scope, value) {
  goog.net.cookies.set(this.getTokenCookieName_(scope), value, 63072000)
};
...
google.accounts.TokenStore_.prototype.setScope_ = function(value) {
  this.scope_ = value;
  goog.net.cookies.set(this.getScopeCookieName_(), encodeURIComponent(value), 63072000)
};
...
google.accounts.User_.prototype.redirectForLogin_ = function() {
  ...
  if(i >= 0) {
    return
  }var nextUrl = document.location.href, frag = nextUrl.indexOf("#");
  if(frag >= 0) {
    nextUrl = nextUrl.substring(0, frag)
  }google.accounts.log_("redirect fired...");
  top.location.href = google.accounts.EXTERNAL_PREFIX_ + "AuthSubRequestJS?session=1&scope=" + this.store_.getScope_() + "&next=" + encodeURIComponent(nextUrl)
};
...
google.gdata.util.isStringOrStringObject_ = function(value) {
  return goog.isString(value) || value && value.constructor == String
};
  ...
  for(var name in js) {
    if(name.charAt(0) != "$" && name != "__proto__") {
      ...
      if(type == "string" || type == "number" || type == "boolean") {
        name = name.replace(/\$/g, ":");
        atom += " " + name + "='" + google.gdata.util.escapeXml_(value) + "'"
      }
    }
  }if(js.$ns) {
    for(var prefix in js.$ns) {
      var uri = js.$ns[prefix];
      atom += " xmlns" + (prefix ? "" : ":" + prefix) + '="' + uri + '"'
    }
  }atom += ">";
  for(var name in js) {
    if(name.charAt(0) != "$" && name != "__proto__") {
    ...
    atom += google.gdata.util.escapeXmlContent_(js.$t)
  }atom += "</" + rootTag + ">";
  return atom
};
(function() {
  ...
  for(var code = 0;code < 32;code++) {
    if(code != 9 && code != 10 && code != 13) {
      var hex = (code < 16 ? "0" : "") + code.toString(16);
      controlCharacters += "\\x" + hex
    }
  }google.gdata.util.patternOfControlCharacters_ = new RegExp("[" + controlCharacters + "]", "g")
})();
google.gdata.util.removeControlCharacters_ = function(value) {
  return value.replace(google.gdata.util.patternOfControlCharacters_, "")
};
google.gdata.util.escapeXmlContent_ = function(s) {
  return s && google.gdata.util.isStringOrStringObject_(s) ? google.gdata.util.removeControlCharacters_(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") : s
};
google.gdata.util.escapeXml_ = function(s) {
  return s && google.gdata.util.isStringOrStringObject_(s) ? google.gdata.util.escapeXmlContent_(s).replace(/"/g, "&quot;").replace(/'/g, "&apos;") : s
};
google.gdata.util.buildNamespaceDictionaries = function(resource) {
  ...
  }if(goog.isArray(resource.entry)) {
    for(var i = 0;i < resource.entry.length;i++) {
      resource.entry[i].$ns = resource.$ns
    }
  }
};
google.gdata.util.getEntryLink = function(entry, rel) {
...
google.gdata.util.JSON_PROPERTY_PATTERN_ = /("[^"]*":")(.*?)(","[^"]*":)/;
google.gdata.util.fixUnescapedDoubleQuotes_ = function(value) {
  value = value.replace(/\}/g, ',"_\u0001":null}');
  for(var nextIndex = 0;;) {
    var currentValue = value.substring(nextIndex), match = google.gdata.util.JSON_PROPERTY_PATTERN_.exec(currentValue);
    if(!match) {
      break
    }var advance = 0;
    if(match[2].indexOf('"') >= 0) {
      var corrected = match[2].replace(/"/g, '\\"'), segmentIndex = match.index + match[1].length, segmentLength = match[2].length;
      advance = segmentIndex + segmentLength;
      value = value.substring(0, nextIndex + segmentIndex) + corrected + value.substring(nextIndex + advance)
    }nextIndex += advance + 2
  }value = value.replace(/,"_\x01":null\}/g, "}");
  return value
...
google.gdata.client.init = function(opt_errorHandler) {
  if(google.gdata.client.isInitialized_) {
    return
  }google.gdata.client.isInitialized_ = true;
  try {
    google.gdata.client.checkSupportedEnvironment_();
    google.gdata.client.checkLoader_()
  }catch(e) {
    if(opt_errorHandler) {
      opt_errorHandler(e)
    }else {
...
google.gdata.client.isInitialized_ = false;
google.gdata.client.checkVersion_ = function(version, minVersion) {
  var v = version.split("."), m = minVersion.split(".");
  for(var i = 0;i < v.length || i < m.length;i++) {
    var nv = i >= v.length ? 0 : Number(v[i]), nm = i >= m.length ? 0 : Number(m[i]);
    if(nv > nm) {
      return true
    }if(nv < nm) {
      return false
...
google.gdata.client.checkSupportedEnvironment_ = function() {
  if(google.gdata.runtime.type === google.gdata.runtime.TYPE.MD) {
    return
    ...
    var minFFVersion = "1.5";
    if(!this.checkVersion_(goog.userAgent.VERSION, minFFVersion)) {
      throw Error("Unsupported Firefox version.  Minimum required version is " + minFFVersion + ".  Continue at your own risk.");
    }
  }else if(goog.userAgent.IE) {
    var minIEVersion = "6.0";
    if(!this.checkVersion_(goog.userAgent.VERSION, minIEVersion)) {
      throw Error("Unsupported Internet Explorer version.  Minimum required version is " + minIEVersion + ".  Continue at your own risk.");
    }
  }else {
    throw Error("Unsupported browser.  Continue at your own risk.");
  }
};
google.gdata.client.checkLoader_ = function() {
  var loader = window.google && window.google.loader;
  if(loader && loader.LoadFailure && loader.OriginalAppPath === window.location.href) {
    throw Error("Developer key validation failed.");
  }
};google.gdata.client.status = {OK:200, NOT_OK:300, BAD_REQUEST:400, FORBIDDEN:403, PRECONDITION_FAILED:412, INTERNAL_SERVER_ERROR:500};google.gdata.client.Uri = function(uriStr) {
  ...
  if(developerKey) {
    ...
    if(e.target.status < google.gdata.client.status.NOT_OK) {
      if(goog.string.isEmpty(e.target.responseText)) {
        ...
        try {
          e.target.responseText = goog.json.parse(e.target.responseText)
        }catch(ex) {
          try {
            var fixedResponseText = google.gdata.util.fixUnescapedDoubleQuotes_(e.target.responseText);
            e.target.responseText = goog.json.parse(fixedResponseText)
          }catch(ex) {
            var error = Error("Invalid JSON format. Unescaped JSON is a not supported format for attribute or text values.");
            if(opt_errorHandler) {
...
google.gdata.client.ClientLoginAuthenticator.prototype.getLoginUri_ = function() {
  if(google.gdata.runtime.type != google.gdata.runtime.TYPE.UP) {
    return this.service_.loginProtocol_ + "://" + this.service_.loginDomain_ + this.LOGIN_PATH
  }else {
...
};google.gdata.client.Service = function(serviceName, applicationName) {
  google.gdata.client.init();
  this.serviceName = serviceName;
  this.applicationName = applicationName;
  this.sessionId_ = (new Date).getTime() + "-" + google.gdata.util.getRandomInt(100, 999);
  this.authenticator_ = new this.DEFAULT_AUTHENTICATOR_(this);
  this.altSupportMap_ = {};
...
google.gdata.client.Service.prototype.DEFAULT_AUTHENTICATOR_ = google.gdata.client.NullAuthenticator;
...
google.gdata.client.Service.prototype.XHR_TRANSPORT_ = new google.gdata.client.XmlHttpRequestTransport;
google.gdata.client.Service.prototype.SCRIPT_TAG_TRANSPORT_ = new google.gdata.client.ScriptTagTransport;
google.gdata.client.Service.prototype.XD_TRANSPORT_ = new google.gdata.client.XdTransport;
...
google.gdata.client.Service.prototype.getCrossDomainTransport_ = function(uri, method) {
  if(method == "GET" && !this.isAuthenticationRequired_(uri)) {
    return this.SCRIPT_TAG_TRANSPORT_
  }else if(this.isXd2Supported()) {
    if(this.hasDummyResource_()) {
      return this.XD_TRANSPORT_
    }else {
      var message = "An image of the same domain is required on this page for authenticated reads and all writes.";
      throw Error(message);
    }
  }else {
...
google.gdata.client.Service.prototype.hasDummyResource_ = function() {
  if(google.gdata.client.Service.hasDummyResource_ == undefined) {
    if(google.gdata.runtime.type == google.gdata.runtime.TYPE.UP) {
      var images = document.getElementsByTagName("img");
      for(var i = 0;i < images.length;i++) {
        var image = images[i];
        if(goog.Uri.haveSameDomain(image.src, window.location.href)) {
          google.gdata.client.Service.hasDummyResource_ = true;
          return true
        }
      }google.gdata.client.Service.hasDummyResource_ = false
    }else {
      google.gdata.client.Service.hasDummyResource_ = true
    }
  }return google.gdata.client.Service.hasDummyResource_
};
...
google.gdata.client.Service.prototype.sendRequestWithRetry_ = function(method, uri, data, headers, continuation, opt_errorHandler, opt_retry) {
  if(this.isAuthenticationRequired_(uri)) {
    this.authenticator_.setAuthHeaders(uri, headers)
  ...
  if(altUsed) {
    var resultHandler = altUsed == google.gdata.client.alt.JSON ? function(req, continuation, opt_errorHandler) {
      if(typeof req.responseText == "string") {
        var contentType = req.getResponseHeader("Content-Type"), js = contentType && contentType.indexOf("xml") > 0 ? google.gdata.util.convertXmlTextToJavaScript(req.responseText) : google.gdata.util.convertJsonTextToJavaScript(req.responseText);
...
google.gdata.client.Service.prototype.sendRequestToTransport_ = function(transport, method, uri, alt, data, headers, resultHandler, continuation, opt_errorHandler, opt_retry) {
  var service = this, actualAlt = transport.getActualAlt(alt), userAgent = encodeURIComponent(this.applicationName + " GData-JavaScript/" + (window.GData_API_Version || "dev") + " " + this.sessionId_);
...
google.gdata.client.Service.prototype.getResource_ = function(uri, continuation, opt_errorHandler, opt_otherParams) {
  if((opt_otherParams && opt_otherParams.authenticationRequired || this.isAuthenticationRequired_(uri)) && !this.authenticator_.isAuthenticated(uri)) {
    var service = this;
    this.authenticator_.authenticate(uri, function() {
...
google.gdata.client.Service.prototype.insertEntry = function(uri, entry, continuation, opt_errorHandler, opt_entryClass) {
  if(this.isAuthenticationRequired_(uri) && !this.authenticator_.isAuthenticated(uri)) {
    var service = this;
    this.authenticator_.authenticate(uri, function() {
...
goog.exportSymbol("google.gdata.client.Service.prototype.setHeaders", google.gdata.client.Service.prototype.setHeaders);google.gdata.client.GoogleService = function(serviceName, applicationName) {
  google.gdata.client.Service.call(this, serviceName, applicationName);
  this.username_ = null;
  this.password_ = null;
  this.altSupportMap_[google.gdata.client.alt.ATOM] = true;
...
google.gdata.client.GoogleService.prototype.DEFAULT_AUTHENTICATOR_ = google.gdata.client.AuthSubAuthenticator;
google.gdata.client.GoogleService.prototype.loginProtocol_ = "https";
google.gdata.client.GoogleService.prototype.loginDomain_ = "www.google.com";
...
google.gdata.client.GoogleService.prototype.setUserCredentials = function(username, password) {
  ...
  if(this.authenticator_.constructor != google.gdata.client.ClientLoginAuthenticator) {
    this.authenticator_ = new google.gdata.client.ClientLoginAuthenticator(this)
  }
};
...
google.gdata.client.GoogleService.prototype.isAuthenticationRequired_ = function(uri) {
  return this.authenticator_.isAuthenticationRequired(uri)
